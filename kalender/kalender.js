...
async function loadBudaya() {
  try {
    ...
    // Render kalender dan gambar default
    renderCalendar();
    setDefaultImage(); 
  } catch (err) {
...
// Tampilkan gambar default saat awal
function setDefaultImage() {
  const mainImg = document.getElementById("mainEventImage");
  const popupImg = document.getElementById("eventImage");

  if (mainImg) mainImg.src = "../img/nusantara.jpg";
  if (popupImg) popupImg.src = "../img/nusantara.jpg";
}
...
function showPopup(event) {
  ...
  document.getElementById("eventImage").src = imgPath;
  ...
  if (!seeDetailsBtn) {
    ...
    seeDetailsBtn.onclick = () => {
      window.location.href = `../detail/detailbudaya.html?id=${event.id}`;
    };
  }
